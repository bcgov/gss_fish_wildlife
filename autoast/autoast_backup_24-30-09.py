...
class AST_FACTORY:
    ...
    def add_job_result(self, job_index, condition):
        ...
        # Update the condition for the specific job (job_index corresponds to the row number). +2 to account for header and 0-index
        ws.cell(row=job_index + 2, column=ast_condition_index, value=condition)  
    ...
    def batch_ast(self):
        ...
        logger.info(f"Number of jobs: {len(self.jobs)}")
          ...
          # Iterate through the jobs and run each with timeout management
        for job in self.jobs:
            try:
                print(f"Starting job {counter}")
                logger.info(f"Starting job {counter}")
                self.run_job_with_timeout(job)
                print(f"Job {counter} processed")
                logger.info(f"Job {counter} processed")
            except Exception as e:
                # Log the exception and mark the job as failed
                print(f"Error encountered with job {counter}: {e}")
                logger.error(f"Error encountered with job {counter}: {e}")
                self.add_job_result(counter - 1, 'Failed')
            finally:
                counter += 1  
